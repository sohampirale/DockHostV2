...
export async function userSignupController(req, res) {
    ...
    try {
        ...
        return res
            ...
            .status(201)
            .json(new ApiResponse(true, "User created successfully"));
    }
    catch (error) {
 ...
 */
export async function createNewInstanceController(req, res) {
    ...
    try {
        ...
        if (user.length == 0) {
            ...
            if (noOfInstancesOfUser > maxNoOfInstancesPerUser) {
                return res.status(400).json(new ApiResponse(false, "You have reached max no of instances limit, delete existing instances to spawn new"));
            }
        }
        const USERNAME = user[0].username;
        console.log('USERNAME : ', USERNAME);
        if (activeBackends.size == 0) {
            return res.status(500).json(new ApiResponse(false, "No active backends right now,start one"));
        }
        for (const [key, value] of activeBackends) {
            ...
            if (!exists) {
                ...
                const timeoutId = setTimeout(() => {
                    timeout = true;
                    return res.status(500).json(new ApiResponse(false, "Response timeout, please wait while your request is being processed"));
        ...
        return res.status(400).json(new ApiResponse(false, "You already have existing instance in all active backends"));
    }
    catch (error) {
        console.log('ERROR : ', error);
        return res.status(500).json(new ApiResponse(false, error?.message || "Something went wrong"));
    }
}
 ...
 */
export async function deleteExistingInstanceController(req, res) {
    const userData = req.data;
    console.log('userData : ', userData);
    try {
        const { backendId } = req.body;
        if (!activeBackends.has(backendId)) {
            return res.status(500).json(new ApiResponse(false, "Requested backend is not connected at the moment try again later"));
        }
        ...
        const timeoutId = setTimeout(() => {
            timeout = true;
            return res.status(500).json(new ApiResponse(false, "Timeout deleting the container, wait for few minutes"));
        }, MAX_CLIENT_WAITING_TIME);
        await asyncSocketAwaiter(socket, USERNAME, async (exists) => {
            if (timeout) {
                return;
            }
            else {
            ...
            if (!exists) {
                return res.status(400).json(new ApiResponse(false, "You dont have instance on this backend"));
            }
 ...
 */
export async function resumeExistingInstanceController(req, res) {
    const userData = req.data;
    console.log('userData : ', userData);
    try {
        const { backendId } = req.body;
        ...
        const timeoutId = setTimeout(() => {
            timeout = true;
            return res.status(500).json(new ApiResponse(false, "Timeout checking status of the container, wait for few minutes"));
        }, MAX_CLIENT_WAITING_TIME);
        await asyncSocketAwaiter(socket, USERNAME, async (exists) => {
            if (timeout) {
                return;
            ...
            await asyncSocketAwaiter(socket, USERNAME, async (running) => {
                if (timeout) {
                    return;
                }
                else {
                ...
                if (running) {
                    return res.status(200).json(new ApiResponse(false, "Instance is already running"));
                }
                ...
                await asyncSocketAwaiter(socket, USERNAME, async (data) => {
                    ...
                    if (success) {
                        ...
                        return res.status(status).json(new ApiResponse(true, "Instance resumed successfuly"));
                    }
                });
            });
        });
 ...
 */
export async function stopExistingInstanceController(req, res) {
    const userData = req.data;
    console.log('userData : ', userData);
    try {
        const { backendId } = req.body;
        ...
        await asyncSocketAwaiter(socket, USERNAME, async (exists) => {
            ...
            await asyncSocketAwaiter(socket, USERNAME, async (running) => {
                ...
                if (!running) {
                    return res.status(200).json(new ApiResponse(false, "Instance is already stopped"));
                }
